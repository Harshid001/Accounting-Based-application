...
export const GET = withAuth(async (req: NextRequest, { user }) => {
  try {
    ...
    const data = await getRevenueReportData(user.id, user.role, startDate, endDate, clientId);
    ...
    throw error; // Let withAuth's error handler deal with unexpected errors
  }
});
